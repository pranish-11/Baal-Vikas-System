...
export function AppProvider({ children }) {
  ...
  const [messages, setMessages] = useState(() => {
    try {
      ...
      const uid = profile?.user?.id || 'default';
      const deleted = new Set(JSON.parse(localStorage.getItem('axion_deleted_conversations')));
      const msgs = JSON.parse(localStorage.getItem(`axion_messages_${uid}`)) || [];
  ...
  const refreshNotifDot = useCallback(() => {
    let has = false;
    for (const m of messages) { if (m.chat?.length > 0) { const last = m.chat[m.chat.length - 1]; if (last.from === 'in') { has = true; break; } } }
  ...
  const refreshData = useCallback(async () => {
    try {
      ...
      try { 
        const profile = JSON.parse(localStorage.getItem('axion_profile'));
        const uid = profile?.user?.id || 'default';
        const v = localStorage.getItem(`axion_messages_${uid}`); 
  ...
  useEffect(() => {
    const handleStorage = (e) => {
      const profile = JSON.parse(localStorage.getItem('axion_profile') || '{}');
      const uid = profile?.user?.id || 'default';
      if (e.key === `axion_messages_${uid}`) { try { const deleted = new Set(JSON.parse(localStorage.getItem('axion_deleted_conversations'))); setMessages(JSON.parse(e.newValue || '[]').filter(m => !deleted.has(m.id))); } catch {} }
  ...
  useEffect(() => { 
    if (messages.length > 0) {
      try { 
        ...
        localStorage.setItem(`axion_messages_${uid}`, JSON.stringify(messages)); 
        queueSyncToDB(`axion_messages_${uid}`, messages); 
      } catch {} 
    } 
  }, [messages, user]);
  ...
  const sendMsg = useCallback(async (text, fileData) => {
    ...
    setMessages(prev => prev.map(m => {
      ...
      const entry = { from: 'out', text: text || '', time: 'Now' };
      if (fileData) {
