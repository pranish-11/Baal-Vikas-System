...
import { Search, Users, MessageSquare, ClipboardList, X } from 'lucide-react';

export default function GlobalSearch({ open, onClose }) {
  const { students, messages, complaints, navTo } = useApp();
  const [query, setQuery] = useState('');
  const [results, setResults] = useState({ students: [], messages: [], complaints: [] });
  const inputRef = useRef(null);

  useEffect(() => { if (open) { setTimeout(() => inputRef.current?.focus(), 100); } }, [open]);

  useEffect(() => {
    if (!query.trim()) { setResults({ students: [], messages: [], complaints: [] }); return; }
  ...
  }, [query, students, messages, complaints]);
      ...
      <div className="modal-box" style={{ maxWidth: 520, padding: 0, overflow: 'hidden', marginTop: '10vh' }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <input ref={inputRef} className="form-input-m" style={{ flex: 1, border: 'none', outline: 'none', fontSize: 15, fontWeight: 600, background: 'transparent' }}
            placeholder="Search students, messages, complaints..." value={query} onChange={e => setQuery(e.target.value)}
        ...
        <div style={{ maxHeight: 400, overflowY: 'auto', padding: '8px 0' }}>
          {!query ? (
            <div style={{ padding: 40, textAlign: 'center', color: 'var(--text3)', fontSize: 13, fontWeight: 600 }}>Type to search across students, messages, and complaints</div>
          ) : total === 0 ? (
